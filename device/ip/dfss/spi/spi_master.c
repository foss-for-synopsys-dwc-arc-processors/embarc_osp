...
#ifdef IO_SPI_MST0_PRESENT
static _Interrupt void spi_mst0_rx_ISR()   { spi_mst_rx_ISR_proc(0); }
static _Interrupt void spi_mst0_tx_ISR()   { spi_mst_tx_ISR_proc(0); }
static _Interrupt void spi_mst0_idle_ISR() { spi_mst_idle_ISR_proc(0); }
static _Interrupt void spi_mst0_err_ISR()  { spi_mst_err_ISR_proc(0); }
#endif
#ifdef IO_SPI_MST1_PRESENT
static _Interrupt void spi_mst1_rx_ISR()   { spi_mst_rx_ISR_proc(1); }
static _Interrupt void spi_mst1_tx_ISR()   { spi_mst_tx_ISR_proc(1); }
static _Interrupt void spi_mst1_idle_ISR() { spi_mst_idle_ISR_proc(1); }
static _Interrupt void spi_mst1_err_ISR()  { spi_mst_err_ISR_proc(1); }
#endif
#ifdef IO_SPI_MST2_PRESENT
static _Interrupt void spi_mst2_rx_ISR()   { spi_mst_rx_ISR_proc(2); }
static _Interrupt void spi_mst2_tx_ISR()   { spi_mst_tx_ISR_proc(2); }
static _Interrupt void spi_mst2_idle_ISR() { spi_mst_idle_ISR_proc(2); }
static _Interrupt void spi_mst2_err_ISR()  { spi_mst_err_ISR_proc(2); }
#endif
#ifdef IO_SPI_MST3_PRESENT
static _Interrupt void spi_mst3_rx_ISR()   { spi_mst_rx_ISR_proc(3); }
static _Interrupt void spi_mst3_tx_ISR()   { spi_mst_tx_ISR_proc(3); }
static _Interrupt void spi_mst3_idle_ISR() { spi_mst_idle_ISR_proc(3); }
static _Interrupt void spi_mst3_err_ISR()  { spi_mst_err_ISR_proc(3); }
#endif
...
#if __Xdmac
static void create_dma_descriptors_rx(spi_info_pt dev, uint8_t *dest, uint32_t size, uint32_t burst);
static void create_dma_descriptors_tx(spi_info_pt dev, uint8_t *src, uint32_t size, uint32_t burst);
#endif
...
uint32_t io_spi_master_open( uint32_t dev_id )
{
    ...
    h = 0;
    while ((spi_master_devs[h].instID != dev_id) && (spi_master_devs[h].instID != SPI_MAX_CNT))
    {
        h++;
    }
    if ((spi_master_devs[h].instID == SPI_MAX_CNT) || (0 != spi_handles[dev_id]))
    ...
    {
        dev->dmarxdescriptor[1] = dev->dmarxdescriptor[5] = dev->reg_base + DR;
    ...
    {
        dev->dmatxdescriptor[2] = dev->dmatxdescriptor[6] = dev->reg_base + DR;
...
void io_spi_master_read( uint32_t dev_id, uint8_t * data, uint32_t * size )
{
    ...
    REG_WRITE( SPIEN, 0 );
    val = (dev->rx_size + (dev->log_xfer_size << 1) - 1) >> dev->log_xfer_size;
    REG_WRITE( CTRL1, val );
    ...
    {   /* RX descriptor - postpone for SPI_TRANSMIT_RECEIVE_MODE, since size depends on tx-size */
        if ((*size != 0) && ( REG_GET_BITS(CTRL0, 2, 8) != SPI_TRANSMIT_RECEIVE_MODE ))
        {
            /* rx data available; at least one */
            create_dma_descriptors_rx(dev, data, *size, (1 + REG_READ( RXFTLR )));
...
void io_spi_master_write( uint32_t dev_id, uint8_t * data, uint32_t * size )
{
    ...
    if ( REG_GET_BITS(CTRL0, 2, 8) == SPI_TRANSMIT_RECEIVE_MODE )
    {
        ...
        if ((dev->dmatxchanid != DMA_NONE) && (dev->rx_size != 0))
        {
            ...
            create_dma_descriptors_rx(dev, (uint8_t *)dev->rx_buff , dev->rx_size, (1 + REG_READ( RXFTLR )));
    ...
    {   /* TX descriptor */
        if (*size != 0)
        {
            ...
            create_dma_descriptors_tx(dev, data, *size, (dev->fifo_depth - REG_READ( TXFTLR )));
...
void io_spi_master_ioctl( uint32_t dev_id, uint32_t cmd, void * arg )
{
    ...
    switch (cmd)
    {
        case IO_SET_CB_RX:   dev->rx_cb  = ((io_cb_t *) arg)->cb; break;
        case IO_SET_CB_TX:   dev->tx_cb  = ((io_cb_t *) arg)->cb; break;
        case IO_SET_CB_ERR:  dev->err_cb = ((io_cb_t *) arg)->cb; break;

        case IO_SPI_MASTER_SET_SE:            REG_WRITE( SER, *((uint32_t *)arg) ); break;
        case IO_SPI_MASTER_SET_RX_THRESHOLD:  REG_WRITE( RXFTLR, *((uint32_t *) arg) ); break;
        case IO_SPI_MASTER_SET_TX_THRESHOLD:  REG_WRITE( TXFTLR, *((uint32_t *) arg) ); break;
        case IO_SPI_MASTER_GET_XFER_SIZE:     *((uint32_t*)arg) = ( 8 << dev->log_xfer_size); break;
        ...
        case IO_SPI_MASTER_SET_PACKING:
            {   /* DFS must be set before applying this option */
                ...
                {   /* set packing */
                    if ( dev->max_xfer_size == 32 )
                    {   /* 32-bits SPI */
                        regval = (regval >> 16) & 0x01F;    // Get DFS field from CTRL0 value
                        dev->log_xfer_size = ( regval == 15 ) ? 1 : ( regval == 7 ) ? 0 : dev->max_xfer_size >> 4;
                    }
                    else
                    {   /* 16-bits SPI */
                        regval = regval & 0x00F;            // Get DFS field from CTRL0 value
                        dev->log_xfer_size = ( regval == 7 ) ? 0 : dev->max_xfer_size >> 4;
                    }
                }
                break;
            }
        ...
        default:
        {
            ...
            switch (cmd)
            {
                case IO_SPI_MASTER_SET_BAUD :           REG_WRITE( BAUDR, *((uint32_t *)arg) ); break;
                case IO_SPI_MASTER_SET_RX_DELAY:        REG_WRITE( RX_SAMPLE_DLY, *((uint32_t *) arg) ); break;
                case IO_SPI_MASTER_SET_TRANSFER_MODE:   REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 2, 8 ); break;
                case IO_SPI_MASTER_SET_SCPH:            REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 6 ); break;
                case IO_SPI_MASTER_SET_SCPL:            REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 7 ); break;
                case IO_SPI_MASTER_SET_SRL_MODE:        REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 11 ); break;

                case IO_SPI_MASTER_SET_DFS: REG_WRITE_BITS( CTRL0, regval, ((*((uint32_t *)arg)) - 1), (3+dev->log_xfer_size), ((dev->log_xfer_size - 1) << 4) ); break;
...
static void spi_mst_rx_ISR_proc(uint32_t dev_id)
{
    ...
    if (dev->dmarxchanid == DMA_NONE)
    {
        ...
        {   /* read buffer completely filled, mask RX-avail interrupt */
            val = REG_READ( IMR );
            REG_WRITE( IMR, (val & ~R_RX_FULL) );
        }
#if __Xdmac
    }
    else if ((dev->dmarxchanid >= DMAC_MEM_CHAN_CNT) && (dev->dmarxdescriptor[3] != 0))
    {   /* dma aux - chain not completed */
        _dma_chan_enable((0x1 << dev->dmarxchanid), 0);
        _dma_int_clear((0x1 << dev->dmarxchanid));

        _dma_chan_desc_aux(dev->dmarxchanid, &(dev->dmarxdescriptor[4]));
    ...
    else
    {
        ...
        if (remainder != 0)
        {
            ...
            if (dev->log_xfer_size == 1)
            {
                ((uint16_t *)(dev->rx_buff))[ divisor ] = (((uint16_t *)(dev->rx_buff))[ divisor ] & ~mask) | (dev->dmarx_last16 & mask);
            }
            else if (dev->log_xfer_size == 2)
            {
                ((uint32_t *)(dev->rx_buff))[ divisor ] = (((uint32_t *)(dev->rx_buff))[ divisor ] & ~mask) | (dev->dmarx_last32 & mask);           
            }
...
static void spi_mst_tx_ISR_proc( uint32_t dev_id )
{
    ...
    else if ((dev->dmatxchanid >= DMAC_MEM_CHAN_CNT) && (dev->dmatxdescriptor[3] != 0))
    {   /* dma aux - chain not completed */
        _dma_chan_enable((0x1 << dev->dmatxchanid), 0);
        _dma_int_clear((0x1 << dev->dmatxchanid));

        _dma_chan_desc_aux(dev->dmatxchanid, &(dev->dmatxdescriptor[4]));
...
static void spi_mst_idle_ISR_proc( uint32_t dev_id )
{
    ...
    if (dev->handling_tx == 1)
    {
        ...
        if( NULL != dev->tx_cb  ) { dev->tx_cb( dev_id ); }
    ...
    if (dev->handling_rx == 1)
    {
        ...
        if( NULL != dev->rx_cb  ) { dev->rx_cb( dev_id ); }
...
static void fill_txfifo(spi_info_pt dev)
{
    ...
    {   /* 16-bits data */
        ...
        if (2*h != cnt)
        {
            REG_WRITE( DR, ((((uint16_t *)(dev->tx_buff))[ dev->tx_count++ ]) & 0x00ff ) );
        }
    }
    else
    {   /* log_xfer_size == 2; 32-bits data */
        for (h = 0; h != cnt/4; h++)
        {
        ...
        {   uint32_t  mask32 = ~((~0x0) << ((cnt-4*h)*8));
            REG_WRITE( DR, ((((uint32_t *)(dev->tx_buff))[ dev->tx_count++ ]) & mask32 ) );
        }
    }
...
static void retrieve_rxfifo(spi_info_pt dev)
{
    ...
    {   /* 8-bits data */
        for (h = 0; h != cnt; h++)
        {
            ((uint8_t*)(dev->rx_buff))[ dev->rx_count++ ] = (uint8_t)REG_READ( DR );
        }
    } else if (dev->log_xfer_size == 1)
    {   /* 16-bits data */
        for (h = 0; h != cnt/2; h++)
        {
            ((uint16_t*)(dev->rx_buff))[ dev->rx_count++ ] = (uint16_t)REG_READ( DR );
        }
        /* if 2h != cnt: partial rx left at the end */
        if (2*h != cnt)
        {
            ((uint16_t*)(dev->rx_buff))[ dev->rx_count ] = (((uint16_t*)(dev->rx_buff))[ dev->rx_count ] & ~0x00ff) | (REG_READ( DR ) & 0x00ff);
            dev->rx_count++;
        }
    ...
    {   /* log_xfer_size == 2; 32-bits data */
        ...
        {
            uint32_t  mask32 = ~((~0x0) << ((cnt-4*h)*8));
            ((uint32_t*)(dev->rx_buff))[ dev->rx_count ] = (((uint32_t*)(dev->rx_buff))[ dev->rx_count ] & ~mask32) | (REG_READ( DR ) & mask32);
            dev->rx_count++;
        }
    }
...
static void create_dma_descriptors_rx(spi_info_pt dev, uint8_t *dest, uint32_t size, uint32_t burst)
{
    ...
    if (dev->log_xfer_size == 0)
    {
        dev->dmarxdescriptor[0] = SPI_MST_DMA_RX_END_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((size - 1) << SPI_MST_DMA_CTRL_SIZE_POS);
        dev->dmarxdescriptor[2] = (uint32_t)dest + (size - 1);
        if (dev->dmarxchanid >= DMAC_MEM_CHAN_CNT) { dev->dmarxdescriptor[3] = 0; }
    }
    else if ((divisor > 0) && (remainder == 0))
    {
        dev->dmarxdescriptor[0] = SPI_MST_DMA_RX_END_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                          | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        dev->dmarxdescriptor[2] = (uint32_t)dest + (size - 1);
        if (dev->dmarxchanid >= DMAC_MEM_CHAN_CNT) { dev->dmarxdescriptor[3] = 0; }
    }
    else if ((divisor > 0) && (remainder > 0))
    {
        dev->dmarxdescriptor[0] = SPI_MST_DMA_RX_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                      | ((size - 1 - remainder) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                      | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        dev->dmarxdescriptor[2] = (uint32_t)dest + (size - 1 - remainder);
        if (dev->dmarxchanid >= DMAC_MEM_CHAN_CNT)
        {
            dev->dmarxdescriptor[0] |= SPI_MST_DMA_FORCE_INT;
            dev->dmarxdescriptor[3] = (uint32_t)&(dev->dmarxdescriptor[4]);
        }
        dev->dmarxdescriptor[4+0] = SPI_MST_DMA_RX_REM_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                            | ((2 * dev->log_xfer_size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                            | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        if (dev->log_xfer_size == 1) {
            dev->dmarxdescriptor[4+2] = (uint32_t)&(dev->dmarx_last16) + 1;
        } else {
            dev->dmarxdescriptor[4+2] = (uint32_t)&(dev->dmarx_last32) + 3;
        }
        if (dev->dmarxchanid >= DMAC_MEM_CHAN_CNT) { dev->dmarxdescriptor[4+3] = 0; }
    }
    else /* divisor == 0, remainder > 0 */
    {
        dev->dmarxdescriptor[0] = SPI_MST_DMA_RX_REM_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((2 * dev->log_xfer_size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                          | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        if (dev->log_xfer_size == 1) {
...
static void create_dma_descriptors_tx(spi_info_pt dev, uint8_t *src, uint32_t size, uint32_t burst)
{
    ...
    if (dev->log_xfer_size == 1)
    {
        dev->dmatx_last16 = ((uint16_t *)(dev->tx_buff))[ divisor ] & ~((~0x0) << (remainder*8));
    }
    else if (dev->log_xfer_size == 2)
    {
        dev->dmatx_last32 = ((uint32_t *)(dev->tx_buff))[ divisor ] & ~((~0x0) << (remainder*8));
    }

    if (dev->log_xfer_size == 0)
    {
        dev->dmatxdescriptor[0] = SPI_MST_DMA_TX_END_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((size - 1) << SPI_MST_DMA_CTRL_SIZE_POS);
        dev->dmatxdescriptor[1] = (uint32_t)src + (size - 1);
        if (dev->dmatxchanid >= DMAC_MEM_CHAN_CNT) { dev->dmatxdescriptor[3] = 0; }
    } else if ((divisor > 0) && (remainder == 0))
    {
        dev->dmatxdescriptor[0] = SPI_MST_DMA_TX_END_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
    ...
    {
        dev->dmatxdescriptor[0] = SPI_MST_DMA_TX_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                      | ((size - 1 - remainder) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                      | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        dev->dmatxdescriptor[1] = (uint32_t)src + (size - 1 - remainder);
        if (dev->dmatxchanid >= DMAC_MEM_CHAN_CNT)
        {
            dev->dmatxdescriptor[0] |= SPI_MST_DMA_FORCE_INT;
            dev->dmatxdescriptor[3] = (uint32_t)&(dev->dmatxdescriptor[4]);
        }
        dev->dmatxdescriptor[4+0] = SPI_MST_DMA_TX_REM_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                            | ((2 * dev->log_xfer_size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                            | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        if (dev->log_xfer_size == 1) {
            dev->dmatxdescriptor[4+1] = (uint32_t)&(dev->dmatx_last16) + 1;
        } else {
            dev->dmatxdescriptor[4+1] = (uint32_t)&(dev->dmatx_last32) + 3;
        }
        if (dev->dmatxchanid >= DMAC_MEM_CHAN_CNT) { dev->dmatxdescriptor[4+3] = 0; }
    }
    else /* divisor == 0, remainder > 0 */
    {
        dev->dmatxdescriptor[0] = SPI_MST_DMA_TX_REM_CTRL | (burst << SPI_MST_DMA_CTRL_XFER_POS)
                                                          | ((2 * dev->log_xfer_size - 1) << SPI_MST_DMA_CTRL_SIZE_POS)
                                                          | ((2 * dev->log_xfer_size + 1) << SPI_MST_DMA_CTRL_DWINC_POS);
        if (dev->log_xfer_size == 1) {
