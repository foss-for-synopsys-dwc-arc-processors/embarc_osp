...
#ifdef IO_SPI_SLV0_PRESENT
static _Interrupt void spi_slv0_rx_ISR()   { spi_slv_rx_ISR_proc(0); }
static _Interrupt void spi_slv0_tx_ISR()   { spi_slv_tx_ISR_proc(0); }
static _Interrupt void spi_slv0_idle_ISR() { spi_slv_idle_ISR_proc(0); }
static _Interrupt void spi_slv0_err_ISR()  { spi_slv_err_ISR_proc(0); }
#endif
#ifdef IO_SPI_SLV1_PRESENT
static _Interrupt void spi_slv1_rx_ISR()   { spi_slv_rx_ISR_proc(1); }
static _Interrupt void spi_slv1_tx_ISR()   { spi_slv_tx_ISR_proc(1); }
static _Interrupt void spi_slv1_idle_ISR() { spi_slv_idle_ISR_proc(1); }
static _Interrupt void spi_slv1_err_ISR()  { spi_slv_err_ISR_proc(1); }
#endif
#ifdef IO_SPI_SLV2_PRESENT
static _Interrupt void spi_slv2_rx_ISR()   { spi_slv_rx_ISR_proc(2); }
static _Interrupt void spi_slv2_tx_ISR()   { spi_slv_tx_ISR_proc(2); }
static _Interrupt void spi_slv2_idle_ISR() { spi_slv_idle_ISR_proc(2); }
static _Interrupt void spi_slv2_err_ISR()  { spi_slv_err_ISR_proc(2); }
#endif
#ifdef IO_SPI_SLV3_PRESENT
static _Interrupt void spi_slv3_rx_ISR()   { spi_slv_rx_ISR_proc(3); }
static _Interrupt void spi_slv3_tx_ISR()   { spi_slv_tx_ISR_proc(3); }
static _Interrupt void spi_slv3_idle_ISR() { spi_slv_idle_ISR_proc(3); }
static _Interrupt void spi_slv3_err_ISR()  { spi_slv_err_ISR_proc(3); }
#endif
...
uint32_t io_spi_slave_open( uint32_t dev_id )
{
	...
	h = 0;
	while ((spi_slave_devs[h].instID != dev_id) && (spi_slave_devs[h].instID != SPI_MAX_CNT))
	{
		h++;
	}
	if ((spi_slave_devs[h].instID == SPI_MAX_CNT) || (0 != spi_handles[dev_id]))
	...
	if (dev->max_xfer_size == 16 )
	{	/* 16-bits SPI; log_xfer_size == 1 */
...
void io_spi_slave_ioctl( uint32_t dev_id, uint32_t cmd, void * arg )
{
	...
	switch (cmd)
	{
		...
		case IO_SPI_SLAVE_SET_RX_THRESHOLD:  REG_WRITE( RXFTLR, *((uint32_t *) arg) ); break;
		case IO_SPI_SLAVE_SET_TX_THRESHOLD:  REG_WRITE( TXFTLR, *((uint32_t *) arg) ); break;
		...
		default:
		{
			...
			switch (cmd)
			{
				case IO_SPI_SLAVE_SET_TRANSFER_MODE:   REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 2, 8 ); break;
				case IO_SPI_SLAVE_SET_SCPH:            REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 6 ); break;
				case IO_SPI_SLAVE_SET_SCPL:            REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 7 ); break;
				case IO_SPI_SLAVE_SET_SRL_MODE:        REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 11 ); break;					
				case IO_SPI_SLAVE_SET_OUTPUT_ENABLE:   REG_WRITE_BITS( CTRL0, regval, *((uint32_t *)arg), 1, 10 ); break;					

				case IO_SPI_SLAVE_SET_DFS: REG_WRITE_BITS( CTRL0, regval, ((*((uint32_t *)arg)) - 1), (3+dev->log_xfer_size), ((dev->log_xfer_size - 1) << 4) ); break;
...
static void spi_slv_idle_ISR_proc( uint32_t dev_id )
{
	...
	{
		if ((dev->tx_size <= (dev->tx_count << dev->log_xfer_size)) || (dev->dmatxchanid != DMA_NONE))
	...
	if (dev->handling_rx == 1)
	{
		...
		if ((dev->rx_size <= (dev->rx_count << dev->log_xfer_size)) || (dev->dmarxchanid != DMA_NONE))
		{   /* buffer completely processed */
...
static void create_dma_descriptors_rx(spi_info_pt dev, uint8_t *dest, uint32_t size, uint32_t burst)
{
	...
	if (dev->log_xfer_size == 0)
	{
		dev->dmarxdescriptor[0] = SPI_SLV_DMA_RX_END_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
	...
	{
		dev->dmarxdescriptor[0] = SPI_SLV_DMA_RX_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
		...
		dev->dmarxdescriptor[4+0] = SPI_SLV_DMA_RX_REM_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
	...
	{
		dev->dmarxdescriptor[0] = SPI_SLV_DMA_RX_REM_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
														  | ((2 * dev->log_xfer_size - 1) << SPI_SLV_DMA_CTRL_SIZE_POS)
														  | ((2 * dev->log_xfer_size + 1) << SPI_SLV_DMA_CTRL_DWINC_POS);
		if (dev->log_xfer_size == 1) {
...
static void create_dma_descriptors_tx(spi_info_pt dev, uint8_t *src, uint32_t size, uint32_t burst)
{
	...
	if (dev->log_xfer_size == 0)
	{
		dev->dmatxdescriptor[0] = SPI_SLV_DMA_TX_END_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
														  | ((size - 1) << SPI_SLV_DMA_CTRL_SIZE_POS);
	...
	{
		dev->dmatxdescriptor[0] = SPI_SLV_DMA_TX_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
													  | ((size - 1 - remainder) << SPI_SLV_DMA_CTRL_SIZE_POS)
													  | ((2 * dev->log_xfer_size + 1) << SPI_SLV_DMA_CTRL_DWINC_POS);
		...
		dev->dmatxdescriptor[4+0] = SPI_SLV_DMA_TX_REM_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
															| ((2 * dev->log_xfer_size - 1) << SPI_SLV_DMA_CTRL_SIZE_POS)
															| ((2 * dev->log_xfer_size + 1) << SPI_SLV_DMA_CTRL_DWINC_POS);
		if (dev->log_xfer_size == 1) {
	...
	{
		dev->dmatxdescriptor[0] = SPI_SLV_DMA_TX_REM_CTRL | (burst << SPI_SLV_DMA_CTRL_XFER_POS)
														  | ((2 * dev->log_xfer_size - 1) << SPI_SLV_DMA_CTRL_SIZE_POS)
														  | ((2 * dev->log_xfer_size + 1) << SPI_SLV_DMA_CTRL_DWINC_POS);
		if (dev->log_xfer_size == 1) {
