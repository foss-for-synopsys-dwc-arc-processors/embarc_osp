...
coap_pdu_from_pbuf(struct pbuf *pbuf)
{
  ...
  align = (unsigned int)data % 4;
  u8_t header_error = pbuf_header(pbuf, sizeof(coap_pdu_t) + align);
  LWIP_ASSERT("CoAP PDU header does not fit in existing header space", header_error == 0);

  result = (coap_pdu_t *)pbuf->payload;

  memset((void *)result, 0, sizeof(coap_pdu_t));

  result->max_size = pbuf->tot_len - sizeof(coap_pdu_t) - align;
  result->length = pbuf->tot_len - sizeof(coap_pdu_t) - align;
  result->hdr = data;
  result->pbuf = pbuf;
