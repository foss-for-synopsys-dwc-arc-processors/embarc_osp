...
static void ihex_begin_read(struct ihex_state *ihex);
...
extern void ihex_read_at_address(struct ihex_state *ihex,
						  ihex_address_t address);

// Read a single character
static ihex_err_t ihex_read_byte(struct ihex_state *ihex, char chr);

// Read `count` bytes from `data`
extern ihex_err_t ihex_read_bytes(struct ihex_state * restrict ihex,
					 const char * restrict data,
					 ihex_count_t count);

// End reading (may call `ihex_data_read` if there is data waiting)
static ihex_err_t ihex_end_read(struct ihex_state *ihex);
...
//	  }
//
extern ihex_err_t ihex_data_read(struct ihex_state *ihex,
								  ihex_record_type_t type,
